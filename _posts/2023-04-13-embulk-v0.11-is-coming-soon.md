...
First, as of today, [Embulk v0.10.49](https://github.com/embulk/embulk/releases/tag/v0.10.49) has been released.

This v0.10.49 is intended to be the final version of the v0.10 series, and is effectively the Release Candidate for v0.11.0. If no problems are found with it, v0.11.0 will be released the same as v0.10.49.

> As of publishing this article, we intended to have v0.10.48 as the final version. But, it turned out after the publication that it still had a problem with newer Java, then we released v0.10.49 as for the fix.
{: .block-info }

So, please try v0.10.49 before v0.11.0 is released. And, if you find any problem with v0.10.49, please report it to our [User forum](https://github.com/orgs/embulk/discussions/categories/user-forum). We plan to release v0.11.0 in June 2023 if no significant problems are found.
...
After v0.11.0 is officially released, it could happen that "the part of the Embulk core can no longer be changed", and there would be no way. Please try v0.10.49 before it happens.

## Plugin's v0.11 support

As mentioned above, many plugins are expected not to work with v0.10.49 (v0.11.0). Most of them should work again if the plugins are updated to support v0.11. Please contact the developer of each plugin.
...
* Download from a version-specific URL like: `https://dl.embulk.org/embulk-X.Y.Z.jar`
    ...
    * Ex: [https://dl.embulk.org/embulk-0.10.49.jar](https://dl.embulk.org/embulk-0.10.49.jar)
...
Jfyi, the file of Embulk v0.10.49 (`embulk-0.10.49.jar`) is 6.39 MB, while Embulk v0.9.24 (`embulk-0.9.24.jar`) was 42.4 MB. It is much smaller.
...
You can still run Embulk like `$ embulk run ...` as of v0.10.49 and v0.11.0. You don't need to change the command line immediately at the same time as trying v0.10.49 or migrating to v0.11. However, a command line like `$ embulk run ...` will become unavailable during the v0.11 series shortly. Please now start preparing your startup script, or command line starting with `java`.
...
Unfortunately, Embulk supports only Java 8 officially, as of v0.10.49 and v0.11.0. However, we believe that Embulk should work basically with Java 11 and Java 17 at this point. If you are interested, please give it a try.
...
There are several possible locations for `embulk.properties` (see below). But as a starting point when migrating to v0.10.49 or v0.11, an easy option is to start from `~/.embulk/embulk.properties` under `~/.embulk/`. `~/.embulk/` is the directory where plugins have been installed even until v0.9.

## JRuby

Embulk v0.10.49 and v0.11.0 do not include JRuby. If you wanted to use an Embulk plugin in the Ruby gem format (in almost every case at this time), you need to [download JRuby](https://www.jruby.org/download) separately, and configure your Embulk System Properties to use it. (See below in particular.)
...
This is the first setup to use Ruby gem-format plugins in Embulk v0.10.49 and v0.11.
...
````
# If Embulk is v0.10.49, embulk.gem must be 0.10.49, too.
...
Embulk v0.10.49 and v0.11.0 have a directory setting called "Embulk home". This is mostly equivalent to `~/.embulk/` up to v0.9, and still defaults to `~/.embulk/`. In the example of Embulk System Properties above, we started from putting `embulk.properties` in `~/.embulk/`.
...
1. Embulk home can be set from an Embulk System Property `embulk_home` in a command line option `-X`.
    ...
    * `java -jar embulk-0.10.49.jar -Xembulk_home=/var/tmp/foo run ...` (absolute path)
    * `java -jar embulk-0.10.49.jar -Xembulk_home=.embulk2/bar run ...` (repative path)
2. Embulk home can be set from an environment variable `EMBULK_HOME`.
    * It must be an absolute path.
    * `env EMBULK_HOME=/var/tmp/baz java -jar embulk-0.10.49.jar run ...` (absolute path)
...
1. The directories to install Ruby gem-format plugins, and the directory to install Maven-format plugins can be set from Embulk System Properties `gem_home`, `gem_path`, and `m2_repo` in command line options `-X`.
    ...
    * `java -jar embulk-0.10.49.jar -Xgem_home=/var/tmp/bar gem install ...` (absolute path)
    * `java -jar embulk-0.10.49.jar -Xm2_repo=.m2/repository run ...` (relative path)
...
3. The directories to install Ruby gem-format plugins, and the directory to install Maven-format plugins can be set from environment variables `GEM_HOME`, `GEM_PATH`, and `M2_REPO`.
    ...
    * `env GEM_HOME=/var/tmp/baz java -jar embulk-0.10.49.jar gem ...` (absolute path)
...
You should be able to run Embulk v0.10.49 with the configurations above. Please try it before v0.11.0 is released in June 2023.
